...
# --- 한국투자증권 API 클래스 (Claude 피드백 반영: tr_id 정밀 수정) ---
class KIS_Trader:
    def __init__(self):
    ...
    def get_balance(self):
        try:
            ...
            headers = {
                ...
                "tr_id":"JTTT3012R" # 해외주식 잔고조회 tr_id
            }
            params = {
                "CANO":self.cano, 
                "ACNT_PRDT_CD":self.acnt_prdt_cd, 
                "OVRS_EXGI":"NAS", # 기본 거래소
                "TR_CRC_CYCD":"USD",
                "CTX_AREA_FK200":"",
                "CTX_AREA_NK200":""
            }
    ...
    def get_current_price(self, ticker):
        try:
            # [수정] 해외주식 현재가 조회 전용 tr_id 및 거래소 코드 규격
            ...
            headers = {
                ...
                "appkey": self.app_key,
                "appsecret": self.app_secret,
                "tr_id": "HHDFS00000300" # 해외주식 현재가 조회 tr_id
            }
            ...
            if res.status_code == 200:
                ...
                else: self.last_error = rj.get('rt_msg', '조회 실패')
...
# --- 데이터 및 모델 (DB Lock 방지 리트라이) ---
@st.cache_data(ttl=3600)
def get_data():
    tickers = ['^GSPC', '^VIX', '^TNX', 'DX-Y.NYB', 'XLK', 'GC=F', 'CL=F', 'QQQ']
    for _ in range(3):
        try:
            # threads=False 및 개별 다운로드로 SQLite Lock 회피
            dfs = []
            for t in tickers:
                dfs.append(yf.download(t, period='5y', progress=False, threads=False)['Close'])
                time.sleep(0.5)
        ...
        if conf >= 70 and pred != 2:
            ...
            if price > 0:
                ...
                if qty >= 1:
                    ...
                    exec_msg = f"🔥 [3x 매수] {ticker} {qty}주" if res.get('rt_cd') == '0' else f"❌ {res.get('rt_msg')}"
                else: exec_msg = "💡 잔고 부족"
            else: exec_msg = f"⚠️ 조회 실패: {trader.last_error}"
        if bot:
            status = "🚀 LONG(3x)" if pred == 1 else "📉 SHORT(3x)" if pred == 0 else "⚪ 보합"
            await bot.send_message(chat_id=chat_id, text=f"🎯 [리포트]\n포지션: {status}\n신뢰도: {conf:.1f}%\n결과: {exec_msg}")

# --- Streamlit UI (민환님의 소중한 모니터링 화면) ---
...
with col1: st.metric("현재 예측 신호", "🚀 LONG(3x)" if pred==1 else "📉 SHORT(3x)" if pred==0 else "⚪ 관망")
with col2: st.metric("AI 신뢰도", f"{max(prob)*100:.1f}%")
with col3: st.metric("데이터 상태", "정상 가동" if not df.empty else "재시도 대기")
...
def calculate_perf(df):
    ...
    return rets.index if not df.empty else [], ai, hold
