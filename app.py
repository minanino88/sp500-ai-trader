...
# ==========================================
# 3. 데이터 엔진 (긴급 보강: repair=True 추가)
# ==========================================
def get_market_data():
    try:
        # [수정] repair=True 옵션 추가하여 데이터 누락 복구 시도
        ...
        if spy_raw.empty or vix_raw.empty:
            return pd.DataFrame(), pd.Series(), pd.Series(), "Data Empty (Check yfinance/Network)"
...
def get_signal(spy_close, monthly, vix_close):
    ...
    if state.get('in_market', True):
        ...
        return "KEEP", "Uptrend", curr_p, state
    else:
...
# ==========================================
# 4. 트레이딩 실행 (GITHUB_ACTIONS & 01:00 긴급 대응 보존)
# ==========================================
async def run_trading():
    ...
    if current_hour == 20:
        ...
        spy_int = yf.download(SIGNAL_TICKER, period='1d', interval='5m', progress=False, auto_adjust=True, repair=True)
        if not spy_int.empty:
            ...
            if spy_ret <= -0.03:
                ...
                if qty > 0:
                    ...
                    if bot: await bot.send_message(chat_id=chat_id, text=f"🚨 [01:00 EMERGENCY] Sold All.")

# ==========================================
# 5. 스트림릿 대시보드 (전 기능 통합)
# ==========================================
def run_dashboard():
    st.set_page_config(page_title="SP500 Watchtower v3.0.3", layout="wide")
    st.sidebar.title("v3.0.3 Master")
    ...
    with st.expander("Strategy Guide & Performance Details"):
        ...
        st.divider()
        st.write("Rules: 2m Down Exit / 2% Rebound Enter / VIX Reversal Enter")
