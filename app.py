...
class KIS_Trader:
    ...
    def get_balance(self):
        ...
        try:
            ...
            # [교정] 잔고 조회 tr_id는 JTTT3007R (J로 시작)
    ...
    def get_holdings(self, ticker=TRADE_TICKER):
        ...
        try:
            ...
            # [교정] 보유량 조회 tr_id는 JTTT3012R (J로 시작)
    ...
    def send_order(self, ticker, qty, side="BUY"):
        ...
        try:
            ...
            # [교정] 실전 주문 tr_id는 TTTT1002U (T로 시작)
            ...
            # [핵심 교정] 민환님이 직접 재점검하신 최신 공식 문서 데이터셋
            data = {
                "CANO": self.cano,
                "ACNT_PRDT_CD": self.acnt_prdt_cd,
                "OVRS_EXCG_CD": "AMEX",       # UPRO(Arca)는 AMEX로 분류
                "PDNO": ticker,
                "ORD_QTY": clean_qty,
                "OVRS_ORD_UNPR": "0",         # 시장가일 때 0
                "ORD_SVR_DVSN_CD": "0",       # 서버 구분 0
                "ORD_DVSN": "00"              # 00 = 시장가
            }
...
# ==========================================
# 3. 데이터 엔진 & 지능형 신호 (Float 교정 및 VIX 역발상)
# ==========================================
def get_market_data():
    try:
...
def get_signal(spy_close, monthly, vix_close):
    ...
    # [교정] 민환님이 직접 잡아주신 Series Ambiguous 에러 방지용 float() 캐스팅
    ...
    # [교정] 비상 탈출 시간 (01시)
        ...
        if not spy_int.empty and (float(spy_int['Close'].iloc[-1]) / float(spy_int['Open'].iloc[0])) - 1 <= -0.03:
            ...
            if qty > 0:
                ...
                if bot: await bot.send_message(chat_id=chat_id, text=f"🚨 [01:00 긴급] 전량 매도 완료\n결과: {str(res_ord)[:200]}")

# ==========================================
# 5. 스트림릿 대시보드 (월별 수익률 바 등 시각화 전수 복구)
# ==========================================
def run_dashboard():
    now_kst = dt.now(KST)
    st.set_page_config(page_title="SP500 Watchtower v3.6.6", layout="wide")
    
    st.sidebar.title("v3.6.6 Master")
